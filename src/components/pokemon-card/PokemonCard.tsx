...
export default function PokemonCard({ pokemon, assets }: Readonly<{ pokemon: FullPokemonType, assets: any }>) {
    const size = {
        size: "small",
        height: 12,
        width: 12
    ...
    return (
        <Grid item xs={12} sm={6} lg={4} xl={3}>
            <Box className={classes.cardContainer} sx={{
                '&:hover': {
                    borderColor: assets.color
                }
            }}>
                <Box className={classes.cardOverlay} sx={{
                    '&::after': {
                ...
                <Grid container direction='row' marginY={'1rem'} marginLeft={'1rem'}>
                    <Grid item xs={6}>
                        ...
                        <Stack width={'70%'} marginTop={'1rem'} direction="column" spacing={1}>
                            <PokemonTypes key={`pokemon-${pokemon.id}`} types={pokemon.types} size={size} />
                        </Stack>
                    </Grid>
